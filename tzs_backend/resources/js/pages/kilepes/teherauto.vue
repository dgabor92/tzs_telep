<template>
    <div class="vld-parent">
        ...
        <card :title="$t('Teherauto_info')">
            <v-list-item v-for="item in filteredTeherauto" :key="item.id" link>
        ...
        <div v-if="dialog" class="dialog">
            <div class="dialog-header">
                <h2 class="dialog-title">{{ selectedItem.rendszam }} Teherautó adatai</h2>
            </div>
...
export default {
    ...
    computed: {
        filteredTeherauto() {
            return this.teherautos.filter((item) => item.kilepes_datuma === null)
        },
    },
    methods: {
        closeDialog() {
        ...
        async updateItem(item) {
            ...
            const { data } = await axios.put(`/api/teherautok`, this.selectedItem)
